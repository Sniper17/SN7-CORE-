...
def get_channel(broadcaster_id):
    ...
    try:
        with conn.cursor() as cur:
            cur.execute('''SELECT broadcaster_user_id, username, currency_name,
            ...
            if not row:
                return None
            keys = ["broadcaster_user_id","username","currency_name",
                    "currency_command","currency_emoji","rank_title",
                    "rank_limit","duel_win_points","duel_loss_points"]
...
def get_player(broadcaster_id, username):
    ...
    try:
        with conn.cursor() as cur:
            cur.execute('''SELECT username, points, streak, duels
                           FROM players
                           WHERE broadcaster_user_id=%s AND username=%s''',
                        (int(broadcaster_id), username))
            row = cur.fetchone()
            return ({"username": row[0], "points": row[1],
                     "streak": row[2], "duels": row[3]} if row else None)
    finally:
        conn.close()
    ...
    try:
        with conn.cursor() as cur:
            cur.execute('''SELECT COUNT(*) + 1
                           ...
                           AND p.points > COALESCE(
                               (SELECT points FROM players
                                WHERE broadcaster_user_id=%s AND username=%s), 0)''',
